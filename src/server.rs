                    ...
                    .wrap(
                        DefaultHeaders::new()
                            .add((
                                ...
                                "default-src 'self'; script-src 'self' 'sha256-hkIUP5VZpQ+CH9Va73b6RJlnUGtVokRUEv+DJuZ14uw=' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' https://assets.lego.com; connect-src 'self'; frame-ancestors 'none'; form-action 'self'; base-uri 'self'",
                            )),
                    )
