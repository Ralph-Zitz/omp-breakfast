...
Read all files in `migrations/` (V1 initial schema through V6 order constraint/index, and any newer migrations), `database_seed.sql`, `src/models.rs`, `src/db/`, `src/handlers/`, `src/routes.rs`, `src/middleware/openapi.rs`, and `frontend/src/app.rs`. Do NOT modify any files — this is analysis only.
