...
- Static files are served with a `Content-Security-Policy` header: `default-src 'self'; script-src 'self' 'sha256-hkIUP5VZpQ+CH9Va73b6RJlnUGtVokRUEv+DJuZ14uw=' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' https://assets.lego.com; connect-src 'self'; frame-ancestors 'none'; form-action 'self'; base-uri 'self'`. The `script-src` uses a SHA-256 hash instead of `'unsafe-inline'` to allow Trunk's inline WASM loader `<script type="module">` while blocking arbitrary inline scripts (XSS). This is possible because `filehash = false` in `Trunk.toml` makes the loader script content deterministic across builds. If Trunk is upgraded and the loader script changes, the hash must be recomputed (build, extract the script from `dist/index.html`, and SHA-256 it). The `style-src` still uses `'unsafe-inline'` because the CONNECT design system requires it. The `font-src` directive includes `https://assets.lego.com` to allow loading the LEGO Typewell proprietary font from the LEGO CDN.
...
3. **Collect and de-duplicate findings** — gather all findings from the subagents (or individual specialist agents) that indicate actionable changes (bugs, missing implementations, convention violations, security issues, stale dependencies, etc.). During consolidation, consult `.claude/resolved-findings.md` to weed out false positives: if a subagent flags something that was already analysed and intentionally resolved, and the current code still reflects that resolution, discard the finding as a false positive. Use `resolved-findings.md` as a guide for identifying false positives, not as absolute authority — if the code has changed since the resolution, the finding may be legitimate even if a similar item appears in resolved history.
